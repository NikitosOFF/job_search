...
def predict_rub_salary_hh(job_title):
    ...
    for page in count(0):
        ...
        for vacancy in page_response.json()['items']:
            ...
            salary_of_various_vacancies_hh.append(predict_salary(salary_from_on_hh, salary_to_on_hh))
        if page >= pages_number:
            break
    salary_statistics_hh['vacancies_found'] = page_response.json()['found']
    salary_statistics_hh['vacancies_processed'] = len(salary_of_various_vacancies_hh)
    salary_statistics_hh['average_salary'] = (
        int(sum(salary_of_various_vacancies_hh) / len(salary_of_various_vacancies_hh)))
